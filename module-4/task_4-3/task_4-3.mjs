...
function cmbTask1CalculateClick() {
  const txtTask1Output = document.getElementById("txtTask1Output"); //txtTask1Output.innerHTML = `Calculating...`;
  const txtRectHeight = document.getElementById("txtRectHeight"); // Get height input
  const txtRectWidth = document.getElementById("txtRectWidth"); // Get width input
  const width = parseInt(txtRectWidth.value); // Parse width as integer
  const height = parseInt(txtRectHeight.value); // Parse height as integer
  const area = width * height; // Calculate area
  const perimeter = 2 * (width + height); // Calculate perimeter
  txtTask1Output.innerHTML = `width: ${width}, height: ${height}`; // Display width and height
  txtTask1Output.innerHTML += `<br/>Circumference = ${perimeter}, Area = ${area}`; // Display perimeter and area
}
...
function txtTask2WordKeyPress(aEvent) {
  const txtTask2Output = document.getElementById("txtTask2Output"); //txtTask2Output.innerHTML = `You pressed the key: ${aEvent.key}`;
  if (aEvent.key === "Enter") { // Enter key pressed
    const word = txtTask2Word.value; // Get the word from the input field
    task2Words.push(word); // Add the word to the array
    txtTask2Output.innerHTML = `You have entered ${task2Words.length} words: <br/>`; // Update the output
    txtTask2Output.innerHTML += task2Words.join(", "); // Display all entered words
    txtTask2Word.value = "";// Clear the input field
  }
}

const txtTask2Word = document.getElementById("txtTask2Word"); // Get the input field
